ssadpm <-
  function(xvars,type,rks,theknots,Etab,pred=FALSE,effect="all",intcpt=TRUE){
    ...
    mfdim=dim(Etab);  nunewr=NA;  nknots=nrow(theknots[[1]])
    if(pred){nxvar=length(xvars)} else{nxvar=length(xvars)-1L}
    xnames=rownames(Etab)[-1]
    xdim=rep(NA,nxvar)
    for(k in 1:nxvar){
      if(any(is.na(xvars[[k]]))==FALSE){
        if(is.na(nunewr)){nunewr=nrow(xvars[[k]])}
        xdim[k]=ncol(xvars[[k]])
      }
    }
    if(intcpt){Kmat=matrix(1,nunewr); Knames="0"} else{Kmat=Knames=NULL}
    Jmats=Jnames=Qmats=NULL;   echk=rowSums(Etab)[-1]    
    
    for(j in 1:mfdim[2]){
      cidx=which(Etab[,j]>0L)-1L
      lencidx=length(cidx)
      if(lencidx==1L){
        if(is.null(rks$qn[[cidx]])==FALSE){
        ...
        if(pred==FALSE){Qmats=cbind(Qmats,rks$qm[[cidx]])}
      } else if(lencidx==2L){
        if(effect=="all" || effect=="lin"){
        ...
        if(effect=="all" || effect=="non"){
          ...
          if(pred==FALSE){Qmats=cbind(Qmats,rks$qm[[cidx[1]]]*rks$qm[[cidx[2]]])}
        ...
        if(effect=="all" || effect=="non"){
          ...
          if(is.null(rks$kn[[cidx[3]]])==FALSE){
            if(is.na(imatch[1])){
              Jmats=cbind(Jmats,rks$jm[[cidx[1]]]*rks$jm[[cidx[2]]]*tcprod(rks$kn[[cidx[3]]],rks$qn[[cidx[3]]]))
              Jnames=c(Jnames,paste(xnames[cidx[1]],xnames[cidx[2]],sep=":"))
              if(pred==FALSE){Qmats=cbind(Qmats,rks$qm[[cidx[1]]]*rks$qm[[cidx[2]]]*tcprod(rks$qn[[cidx[3]]]))}
            } else {
              ...
              if(pred==FALSE){Qmats[,jidx]=Qmats[,jidx]+rks$qm[[cidx[1]]]*rks$qm[[cidx[2]]]*tcprod(rks$qn[[cidx[3]]])}
            }
            if(is.null(rks$kn[[cidx[2]]])==FALSE){
              if(is.na(nidx[1])){
                Jmats=cbind(Jmats,rks$jm[[cidx[1]]]*tcprod(rks$kn[[cidx[2]]],rks$qn[[cidx[2]]])*tcprod(rks$kn[[cidx[3]]],rks$qn[[cidx[3]]]))
                Jnames=c(Jnames,xnames[cidx[1]])
                if(pred==FALSE){Qmats=cbind(Qmats,rks$qm[[cidx[1]]]*tcprod(rks$qn[[cidx[2]]])*tcprod(rks$qn[[cidx[3]]]))}
              } else {
                jidx=(1+(nidx[1]-1)*nknots):(nidx[1]*nknots)
                Jmats[,jidx]=Jmats[,jidx]+rks$jm[[cidx[1]]]*tcprod(rks$kn[[cidx[2]]],rks$qn[[cidx[2]]])*tcprod(rks$kn[[cidx[3]]],rks$qn[[cidx[3]]])
                if(pred==FALSE){Qmats[,jidx]=Qmats[,jidx]+rks$qm[[cidx[1]]]*tcprod(rks$qn[[cidx[2]]])*tcprod(rks$qn[[cidx[3]]])}
              }
            } # end if(is.null(rks$kn[[cidx[2]]])==FALSE)
          ...
          if(is.null(rks$kn[[cidx[2]]])==FALSE){
            if(is.na(imatch[2])){
              ...
              if(pred==FALSE){Qmats=cbind(Qmats,rks$qm[[cidx[1]]]*rks$qm[[cidx[3]]]*tcprod(rks$qn[[cidx[2]]]))}
            } else {
              ...
              if(pred==FALSE){Qmats[,jidx]=Qmats[,jidx]+rks$qm[[cidx[1]]]*rks$qm[[cidx[3]]]*tcprod(rks$qn[[cidx[2]]])}
            }
            if(is.null(rks$kn[[cidx[1]]])==FALSE){
              if(is.na(nidx[3])){
                ...
                if(pred==FALSE){Qmats=cbind(Qmats,rks$qm[[cidx[3]]]*tcprod(rks$qn[[cidx[1]]])*tcprod(rks$qn[[cidx[2]]]))}
              } else {
                ...
                if(pred==FALSE){Qmats[,jidx]=Qmats[,jidx]+rks$qm[[cidx[3]]]*tcprod(rks$qn[[cidx[1]]])*tcprod(rks$qn[[cidx[2]]])}
              }
            } # end if(is.null(rks$kn[[cidx[1]]])==FALSE)
          } # end if(is.null(rks$kn[[cidx[2]]])==FALSE)
          ...
          if(is.null(rks$kn[[cidx[1]]])==FALSE){
            if(is.na(imatch[3])){
              ...
              if(pred==FALSE){Qmats=cbind(Qmats,rks$qm[[cidx[2]]]*rks$qm[[cidx[3]]]*tcprod(rks$qn[[cidx[1]]]))}
            } else {
              ...
              if(pred==FALSE){Qmats[,jidx]=Qmats[,jidx]+rks$qm[[cidx[2]]]*rks$qm[[cidx[3]]]*tcprod(rks$qn[[cidx[1]]])}
            }
            if(is.null(rks$kn[[cidx[3]]])==FALSE){
              if(is.na(nidx[2])){
                Jmats=cbind(Jmats,rks$jm[[cidx[2]]]*tcprod(rks$kn[[cidx[1]]],rks$qn[[cidx[1]]])*tcprod(rks$kn[[cidx[3]]],rks$qn[[cidx[3]]]))
                Jnames=c(Jnames,xnames[cidx[2]])
                if(pred==FALSE){Qmats=cbind(Qmats,rks$qm[[cidx[2]]]*tcprod(rks$qn[[cidx[1]]])*tcprod(rks$qn[[cidx[3]]]))}
              } else {
                jidx=(1+(nidx[2]-1)*nknots):(nidx[2]*nknots)
                Jmats[,jidx]=Jmats[,jidx]+rks$jm[[cidx[2]]]*tcprod(rks$kn[[cidx[1]]],rks$qn[[cidx[1]]])*tcprod(rks$kn[[cidx[3]]],rks$qn[[cidx[3]]])
                if(pred==FALSE){Qmats[,jidx]=Qmats[,jidx]+rks$qm[[cidx[2]]]*tcprod(rks$qn[[cidx[1]]])*tcprod(rks$qn[[cidx[3]]])}
              }
            } # end if(is.null(rks$kn[[cidx[1]]])==FALSE)
          } # end if(is.null(rks$kn[[cidx[2]]])==FALSE)
          ...
          if(pred==FALSE){Qmats=cbind(Qmats,rks$qm[[cidx[1]]]*rks$qm[[cidx[2]]]*rks$qm[[cidx[3]]])}
    ...
    if(is.null(Kmat)==FALSE){colnames(Kmat)=Knames}
    if(is.null(Jmats)==FALSE){colnames(Jmats)=rep(Jnames,each=nknots)}
    return(dps=list(Kmat=Kmat,Jmats=Jmats,Qmats=Qmats))
