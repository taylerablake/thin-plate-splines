predict.ssp <-
  function(object,newdata=NULL,se.fit=FALSE,
    ...
    ### check include
    newnames=include;   oldnames=object$tnames
    lnt=length(newnames)
    ...
    etdim=dim(et);  etnames=rownames(et)[2:etdim[1]]
    etidx=match(etnames,xnames)
    Etab=matrix(0L,nxvar,etdim[2]);   rownames(Etab)=xnames
    ...
    if(is.null(Knames)){
      ...
      yhat=(dps$Jmats%*%kronecker(thvec,diag(nknots)))%*%object$modelspec$coef[(nbf+1):(nbf+nknots)]
      if(se.fit){pse=sqrt(postvar(NULL,dps$Jmats%*%kronecker(thvec,diag(nknots)),object$modelspec$coef.csqrt[(nbf+1):(nbf+nknots),]))}
      ...
      yhat=cbind(dps$Kmat,dps$Jmats%*%kronecker(thvec,diag(nknots)))%*%object$modelspec$coef[c(kidx,(nbf+1):(nbf+nknots))]
      if(se.fit){pse=sqrt(postvar(dps$Kmat,dps$Jmats%*%kronecker(thvec,diag(nknots)),object$modelspec$coef.csqrt[c(kidx,(nbf+1):(nbf+nknots)),]))}
