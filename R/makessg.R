makessg <-
  function(formula,family,data,type=NULL,nknots=NULL,rparm=NA,
    ...
    ysm=sum(yvar);  nunewr=ndpts;  fweights=rep.int(1L,ndpts);  yty=yvar^2
    ...
    if(length(family)==1L){
      ...
      family=family[[1]];   dispersion=NULL
    ...
    if(family==c("poisson")){yvar=matrix(round(yvar));  ymin=min(yvar);  if(ymin[1]<0){stop("Response must be nonnegative integer for family='poisson'.")}}
    if(family==c("negbin")){yvar=matrix(round(yvar));  ymin=min(yvar);  if(ymin[1]<0){stop("Response must be nonnegative integer for family='negbin'.")}}
    if(family=="Gamma"){ymin=min(yvar);  if(ymin[1]<=0){stop("Response must be positive real for family='Gamma'.")}}
    if(family=="inverse.gaussian"){ymin=min(yvar);  if(ymin[1]<=0){stop("Response must be positive real for family='inverse.gaussian'.")}}
    ...
    if(is.na(rparm[1])){
      ...
      yvar=fs[,2];  fweights=fs[,3];   yty=fs[,4];   nunewr=length(yvar)
      if(wtchk){weights=weights[fs[,1]]; if(family=="binomial"){yty=yty/(weights^2)}}
