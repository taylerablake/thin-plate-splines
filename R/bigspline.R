bigspline <-
  function(x,y,type="cub",nknots=30,rparm=0.01,xmin=min(x),
    ...
    type=type[1]
    if(any(type==c("cub","cub0","per"))==FALSE){
      stop("Must set type to 'cub', 'cub0', or 'per'.")
    }
    ...
    if(is.na(rparm)==FALSE){
      ...
      x=as.matrix(seq(0,1,by=rparm));  nunewr=nrow(x)
      yw <- (.Fortran("sumfreq",y,gvec,n,nunewr,double(nunewr),
                      integer(nunewr),PACKAGE="bigsplines"))[5:6]
      widx=which(yw[[2]]>0)
      y=as.matrix(yw[[1]][widx]);  w=yw[[2]][widx];  rm(yw)
      x=as.matrix(x[widx]);        nunewr=nrow(x)
