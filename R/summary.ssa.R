summary.ssa <- 
  function(object,fitresid=TRUE,chunksize=10000,...){
      ...
      if(is.na(object$modelspec$rparm[1])){
        ...
        if(chunksize>=ndpts){
          ...
          if(xseq[lenx]<ndpts) {
            xseq=c(xseq,ndpts+1)
            lenx=lenx+1
          } else {xseq[lenx]=xseq[lenx]+1}
          yhat=NULL;  nxvar=length(object$xvars)
          newdata=vector("list",nxvar);  names(newdata)<-names(object$xvars)
          for(mm in 1:(lenx-1)){
    ...
    sumssa=list(call=object$call,type=object$type,fitted.values=yhat,residuals=object$yvar-yhat,
