gcvgss <-
  function(eta,family,Kmat,Jmat,yvar,
           Qmat,nknots,ndpts,alpha,yty,nbf,
           fweights,weights,maxit,intol,subsamp,dispersion) {
    
    nqmat=matrix(0,nbf+nknots,nbf+nknots)
    nqmat[(nbf+1):(nknots+nbf),(nbf+1):(nknots+nbf)]=ndpts*Qmat
    rm(Qmat);   nunewr=length(yvar)
