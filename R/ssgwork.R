  ...
  function(formula,ssgmk){
    ...
    if(ssgmk$family=="negbin"){
      if(is.null(ssgmk$dispersion)){
        ssgmk$family="poisson";  se.lpold=ssgmk$se.lp;  ssgmk$se.lp=TRUE;  pmod=ssgwork(formula,ssgmk)
        ...
        ssgmk$dispersion=as.numeric(bot/top);  rm(pmod);  ssgmk$family="negbin";  ssgmk$se.lp=se.lpold;  estdisp=TRUE
      } else {estdisp=FALSE}
    } else{estdisp=FALSE}
    ...
    ### get info
    xnames=oldnames;  tnames=tnames;  xdim=ssgmk$xdim
    if(is.null(ssgmk$gcvopts)){
      maxit=5;  gcvtol=10^-5;  alpha=1; inmaxit=100; intol=10^-5; insub=10000;
    } else {
      maxit=ssgmk$gcvopts$maxit;  gcvtol=ssgmk$gcvopts$gcvtol;  alpha=ssgmk$gcvopts$alpha
      inmaxit=ssgmk$gcvopts$inmaxit;  intol=ssgmk$gcvopts$intol;  insub=ssgmk$gcvopts$insub
    }
    ...
    if(ssgmk$skip.iter){
      ...
      csqrt=sqrt(mevar)*fxhat[[2]];  if(ssgmk$family=="negbin"){mevar=1/size}
      iter=vtol=NA
    } else {
      ...
      while(vtol>gcvtol && iter<maxit && min(gammas)>0) {
        ...
        if(ssgmk$nxvar==1L){
          ...
          if(ssgmk$family=="negbin" & estdisp){
            ...
            ssgmk$dispersion=1/size; lambdas=newlam; oldlam=newlam; iter=iter+1L
          } else {vtol=0}
        } else{
    ...
    for(k in 1:ssgmk$nxvar){
      if(any(ssgmk$type[[k]]==c("cub","acub","per"))){
        ...
        if(is.na(ssgmk$rparm[1])==FALSE){ssgmk$xorig[[k]]=as.matrix(ssgmk$flvls[[k]][ssgmk$xorig[[k]]])}
      }
    } # end for(k in 1:ssgmk$nxvar)
    if(is.na(ssgmk$rparm[1])==FALSE){
      yunique=ssgmk$xvars[[ssgmk$nxvar+1]]/as.numeric(ssgmk$fweight); yvar=ssgmk$yorig
      xunique=ssgmk$xvars[1:ssgmk$nxvar];  ssgmk$xvars=ssgmk$xorig;   ssgmk$weights=ssgmk$worig
    } else{xunique=yunique=NA;  yvar=ssgmk$xvars[[ssgmk$nxvar+1]]; ssgmk$xvars=ssgmk$xvars[1:ssgmk$nxvar]}
